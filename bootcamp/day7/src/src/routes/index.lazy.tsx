...
import { getFavouritePokemons, getPokemonSpriteUrlPokemonId, isPokemonInFavourites, removePokemonFromFovourites, searchPokemon } from '@/lib/services/pokemon';
import { useCallback, useDeferredValue, useMemo, useState } from 'react';
import { CommandDialog, CommandEmpty, CommandGroup, CommandItem, CommandList, CommandSeparator, CommandShortcut, CommandInput } from '@/components/ui/command';
...
const FAVOURITE_POKEMONS_SLUG = 'favourite-pokemons';
const emptyPokemonList: Pokemon[] = [];
const columnHelper = createColumnHelper<Pokemon>();

function RouteComponent() {
  const { data: favouritePokemons, isLoading: isLoadingCars, refetch } = useQuery({
    queryKey: [FAVOURITE_POKEMONS_SLUG],
    queryFn: getFavouritePokemons,
  })
          ...
          return (<TableCell className='text-right' >
            <DropdownMenu>
              ...
              <DropdownMenuContent className='w-56'>
                <DropdownMenuGroup>
                  <DropdownMenuItem onClick={() => { removePokemonFromFovourites(info.row.getValue('id')); refetch() }}>
    ...
    <div className='border rounded-lg p-2 flex flex-col grow'>
      ...
      <Table>
        ...
        <TableBody className='h-full'>
          ...
          {!isLoadingCars && table.getRowModel().rows.map(row => (
            <TableRow key={row.id}>
              {row.getVisibleCells().map(cell => <Fragment key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</Fragment>)}
            </TableRow>
          ))}
          {isLoadingCars && repeatAndCollect(10, (i) => <TableRow key={i}><TableCell colSpan={3}><Skeleton className='w-full h-8' /></TableCell></TableRow>)}
        </TableBody>
      </Table>
