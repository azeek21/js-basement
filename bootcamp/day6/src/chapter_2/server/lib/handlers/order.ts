...
function getOrderById(db: DB): Handler {
...
function getAllOrdersOfUser(db: DB): Handler {
	return async (req, res) => {
...
function getAllOrders(db: DB): Handler {
	return async (req, res) => {
		const orders = await db.order.findMany({
			include: {
				items: true,
...
function createOrder(db: DB): Handler {
	return async (req, res) => {
...
function updateOrder(db: DB): Handler {
	return async (req, res) => {
...
function deleteOrder(db: DB): Handler {
	return async (req, res) => {
		const id = Number(req.params['id']);
